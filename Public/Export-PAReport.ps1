                    ...
                    'HTML' {
                        ...
                        if ($Findings.Count -eq 0) {
                            ...
                            foreach ($f in $sortedFindings) {
                                ...
                                [void]$sb.AppendLine("        <tr><td><span class=`"badge severity-$($f.Severity)`">$($f.Severity)</span></td><td title=`"$catLabel`">$catLabel</td><td title=`"$title`">$title</td><td title=`"$principal`">$principal</td><td title=`"$role`">$role</td><td class=`"col-scope`" title=`"$scope`">$scope</td><td class=`"col-rec`" title=`"$rec`">$rec</td><td>$remLabel</td></tr>")
                            }
                        }
                        ...
                        [void]$sb.AppendLine('        })();')
                        # Column resize
                        [void]$sb.AppendLine('        document.querySelectorAll(".findings-table .resizer").forEach(function(resizer) {')
                        [void]$sb.AppendLine('            var startX, startW, th;')
                        [void]$sb.AppendLine('            resizer.addEventListener("mousedown", function(e) {')
                        [void]$sb.AppendLine('                th = resizer.parentElement;')
                        [void]$sb.AppendLine('                startX = e.pageX;')
                        [void]$sb.AppendLine('                startW = th.offsetWidth;')
                        [void]$sb.AppendLine('                document.addEventListener("mousemove", onDrag);')
                        [void]$sb.AppendLine('                document.addEventListener("mouseup", onUp);')
                        [void]$sb.AppendLine('                e.preventDefault();')
                        [void]$sb.AppendLine('            });')
                        [void]$sb.AppendLine('            function onDrag(e) {')
                        [void]$sb.AppendLine('                var w = startW + (e.pageX - startX);')
                        [void]$sb.AppendLine('                if (w > 40) th.style.width = w + "px";')
                        [void]$sb.AppendLine('            }')
                        [void]$sb.AppendLine('            function onUp() {')
                        [void]$sb.AppendLine('                document.removeEventListener("mousemove", onDrag);')
                        [void]$sb.AppendLine('                document.removeEventListener("mouseup", onUp);')
                        [void]$sb.AppendLine('            }')
                        [void]$sb.AppendLine('        });')
                        [void]$sb.AppendLine('    </script>')
                        [void]$sb.AppendLine('</body>')
